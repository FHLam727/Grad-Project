...
class WeiboClient(ProxyRefreshMixin):
    ...
    async def get_all_notes_by_creator_id(
        ...
        crawl_interval: float = 1.0,
        callback: Optional[Callable] = None,
        max_count: int = 50,  # ✏️ CHANGED: 預設只爬最新50條，防止爬晒歷史全部帖文
    ) -> List[Dict]:
        """
        ...
        Args:
            ...
            max_count: Max number of notes to fetch (default 20, set 0 for unlimited)  # ✏️ CHANGED
        ...
        while notes_has_more:
            # ✏️ CHANGED: 夠數就停，唔繼續翻頁爬舊帖
            if max_count > 0 and len(result) >= max_count:
                utils.logger.info(f"[WeiboClient.get_all_notes_by_creator] Reached max_count={max_count} for creator {creator_id}, stopping.")
                break
