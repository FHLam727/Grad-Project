...
from tools.negative_monitor_date import (
    ...
    negative_monitor_note_date_in_range,
    xhs_note_detail_publish_date,
)
...
def _xhs_note_in_negative_monitor_crawl_range(note: dict) -> bool:
    ...
    if not fr and not to_d:
        return True
    d = xhs_note_detail_publish_date(note)
    if d is None:
        return True
    ...
    async def search(self) -> None:
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while accepted < target_max and page <= max_pages_guard:
                ...
                try:
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.search] search Xiaohongshu keyword: {keyword}, page: {page}, "
                    ...
                    # 勿在「本頁仍有筆記」時僅因 has_more=false 就跳過：最後一頁或接口偶發會如此，否則會 0 條入庫
                    if not notes_res:
                        utils.logger.info("[XiaoHongShuCrawler.search] Empty search response, stop.")
                        break
                    raw_items = notes_res.get("items") or []
                    ...
                    post_items = [
                        ...
                        and post_item.get("model_type") not in ("rec_query", "hot_query")
                    ]
                    if not post_items:
                        if not notes_res.get("has_more", False):
                            utils.logger.info("[XiaoHongShuCrawler.search] No notes on page and no more pages.")
                            break
                        utils.logger.info("[XiaoHongShuCrawler.search] Empty note list on page, try next page.")
                        page += 1
                        continue
                    ...
                    for note_detail in note_details:
                        if accepted >= target_max:
                            break
                        if note_detail:
                            if not _xhs_note_in_negative_monitor_crawl_range(note_detail):
                                utils.logger.info(
                                    f"[XiaoHongShuCrawler.search] Skip note outside NEGATIVE_MONITOR crawl date range: "
                    ...
                    if accepted >= target_max:
                        utils.logger.info(
                            f"[XiaoHongShuCrawler.search] Reached target stored notes {accepted}/{target_max}, stop."
                        )
                        break
                    if not has_more:
                        utils.logger.info("[XiaoHongShuCrawler.search] has_more=false, keyword search done for this keyword.")
                        break
